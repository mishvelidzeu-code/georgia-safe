-- The launch allowance of fifty free map reports ends: a non-Premium account
-- gets exactly ONE free report, for the life of the account (no daily reset).
-- Accounts that already used one or more keep their count and are at the limit.
...
begin
  ...
  if not public.has_premium(submitting_user_id) then
    -- Claimed in the same statement as the check, so two concurrent requests
    -- cannot both take the single free report.
    ...
    on conflict (user_id) do update
      set used_count = public.free_place_submission_uses.used_count + 1,
          used_at = now()
      where public.free_place_submission_uses.used_count < 1
